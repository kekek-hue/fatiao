           ...
           ,parseData: function (res) { //res 即为原始返回的数据
            ...
            ,cols: [[ //表头 
                {fixed: 'right',align:'center',title:'操作', toolbar: '#barDemo'}
                    ...
                    //,{type: 'checkbox'}
                    ,{field: 'row', title: '行号',sort: true,hide:true}
                    ,{field: 'id', title: 'ID',sort: true}
                    ,{field: 'title', title: '视频标题'}
                    ,{field: 'coverPath', title: '封面' ,templet:'#faceHtml-img'}
                    // ,{field: 'path', title: '视频' ,templet:'#faceHtml'}
    ...
    //    监听提交
        ...
        table.on('tool(article-table)', function(obj){
            ...
            switch(obj.event){
                ...
                case 'update-attr':     // 修改视频
                   openEdit(data)
                        // location.href = '';
                        // location.href = '#/updateVideo/id='+data.id;
                    break;
                case 'publish-attr':         
                        if (confirm('确认是否发布')) {
        ...
        */
        function videoRelease (id) {
            if (!id) {
                layer.alert('请选择要发布的视频',{icon:1,title:'提示'});return false;
            }
            jq.post('video/videoRelease', {'id':id}, function(resData) {
                 if(resData.code == 200) {
                    layer.alert('发布成功',{icon:1,title:'提示'});
                 } else if (resData.code == 301) {    
                    layer.alert('已发布',{icon:1,title:'提示'});
                 } else {
                     layer.alert('发布失败',{icon:1,title:'提示'});
                 }
            },"json");
        }
