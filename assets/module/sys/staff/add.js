function Lay(){
 layui.use(['layer', 'ppp', 'form', 'iconPicker'], function () {
        ...
        ppp.get('job/lists', {async: false}, function (data) {
            if(data.code == 0){
                let jobData = data.data;
                jobData.forEach(function(value,i){
                    let opt ="<option value="+value.id+">"+value.name+"</option>";
                    $("#job_id").append(opt);
                });
            }
        });
        ppp.get('admin/lists', {async: false}, function (data) {
            if(data.status == 200){
                let userData = data.result.records;
        ...
        form.on('submit(staff-form-submit)', function (data) {
            ...
            if (data.field.id) {
                ...
                ppp.post('staff/addStaff', {data: data.field}, function () {
                    layer.closeAll('loading');
