layui.define(['layer', 'table', 'laytpl'], function (exports) {
 //封装设置css属性方法
   ...
   DragDrop.prototype.init = function(obj) {
       this.stopEvent = obj.stop;
       this.parentBox = document.getElementById(obj.ele)
       this.editSwith = this.parentBox.querySelector('.editOrder');
       this.save = this.parentBox.querySelector('.save');
       this.list = this.parentBox.querySelector('.layui-table-main');
       this.li = this.list.querySelectorAll('.layui-table-body tbody tr');
    //    console.error(this.li);
       //-------------------------------------------------------------------
       var _this = this;
   ...
   DragDrop.prototype.edit = function() {
       ...
       if (!tf) {
           ...
           var cls = aClassNames.toString().replace(reg, ' ');
           this.editSwith.className = cls;
  ...
  layui.link(layui.cache.base + 'dragDrop/dragDrop.css');
